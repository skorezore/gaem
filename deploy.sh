...
if [ "$TRAVIS_PULL_REQUEST" != "false" ]
	then export DROPBOX_SUBFOLDER_NAME=PR#$TRAVIS_PULL_REQUEST-`echo $TRAVIS_REPO_SLUG | sed s:/:_:g | tr [:upper:] [:lower:]`
	else export DROPBOX_SUBFOLDER_NAME=$TRAVIS_BUILD_NUMBER-$TRAVIS_COMMIT_RANGE
fi
